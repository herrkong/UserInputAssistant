...
,_isLooping(false)
{
   ...
   cout<<"EventLoop(Acceptor & acceptor)"<<endl; 
}
...
void EventLoop::waitEpollFd()
{
    ...
    {
        if(_eventList.size() == size_t(nready))
        {
            _eventList.resize(2* nready);//要扩容了。
        }
...
void EventLoop::handleNewConnection()
{
    ...
    conn->setMessageCallback(_onMassage);
...
void EventLoop::doPendingFunctors()
{
    ...
    {
        MutexLockGuard autoLock(_mutex);
        temp.swap(_pendingFunctors);//swap去除多余容量
    }
